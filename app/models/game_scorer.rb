...
class GameScorer
  ...
  def initialize(game_or_hands)
    raise 'Unsupported game type' unless game_or_hands.is_a?(Game) || game_or_hands.all? { |type| type.is_a?(Hand) }

    @hands = game_or_hands.is_a?(Game) ? game_or_hands.hands : game_or_hands
    get_winning_hand
  end
